...
​	之所以遍历set而不是nums，是因为其中可能有很多重复元素

>>>>>>> 96c6eb35d3c2f3b71a1e9c9386f33f5d117757fa
```cpp
class Solution {
public:
    int longestConsecutive(vector<int>& nums) {
...
public:
    vector<int> findAnagrams(string s, string p) {
        ...
        for(int r = 0;r<s.length();r++){
            ...
            cnt_s[s[r]-'a'] += 1;
            if(r<p.length()-1) continue;
            if(cnt_p == cnt_s) res.push_back(l);
            //左端出，l++
            cnt_s[s[l]-'a']-=1;
            l++;
        }
...
<<<<<<< HEAD
        ...
        for(int i = 0;i<nums.size();i++){
            pre+=nums[i];
            if(map.find(pre-k)!=map.end()) res+=map.find(pre-k)->second;
            map[pre]+=1;
=======
        unordered_map<int,int> map;
        map[0] = 1;
        ...
        for(int i = 0;i<nums.size();i++){
            pre+= nums[i];
            if(map.count(pre - k)) res+= map[pre - k];
            map[pre]++;
>>>>>>> 96c6eb35d3c2f3b71a1e9c9386f33f5d117757fa
        }
        return res;
...
class Solution {
public:
    //子集问题与组合和排列问题的区别，每层结果都在结果集中，不只有叶子节点
    vector<int> path;
    vector<vector<int>> res;
    void backtarck(vector<int>& nums,int start){
        res.push_back(path);
...
public:
    string s;
    vector<string> res;
    void backtrack(string& digits,int idx,unordered_map<char,string>& phonemap){
        if(s.length()==digits.length()){
            res.push_back(s);
            return;
        }
        string letter = phonemap.find(digits[idx])->second;
        for(int i = 0;i<letter.length();i++){
            s+=letter[i];
            backtrack(digits,idx+1,phonemap);
            s.erase(s.end()-1);
...
public:
    ...
    vector<vector<int>> res;
    int p = 0;
    void backst(vector<int>& candidates,int target,int idx){
        if(p == target) {
            res.push_back(path);
            return;
            }
...
class Solution {
public:
    bool isr(string s,int begin,int end){
        int l = begin,r = end;
        while(l<r){
...
public:
    ...
    void backtrack(vector<vector<char>>& board,string word,vector<vector<bool>>& visit,int idx,int x,int y){
        ...
        for(int i = 0;i<4;i++){
            int n_x = x+direction[i][0],n_y = y + direction[i][1];
          //错误点在这，先将判断n_x，n_y是否超出界限的条件放到前面，不然会报错
            if(n_x <n && n_x>=0 && n_y<m && n_y>=0 && !visit[n_x][n_y] &&  board[n_x][n_y]==word[idx]){
                visit[n_x][n_y] = true;
                backtrack(board,word,visit,idx+1,n_x,n_y);
                visit[n_x][n_y] = false;
            }
        }
    }
    bool exist(vector<vector<char>>& board, string word) {
        ...
        for(int i = 0;i<n;i++){
            for(int j = 0;j<m;j++){
                if(board[i][j] == word[0]){
                    visit[i][j] = true;
                    backtrack(board,word,visit,1,i,j);
                    visit[i][j] = false;
                }
                if(res) return true;
            }
    ...
    void backtrack(vector<vector<char>>& board, string word, vector<vector<bool>>& visit, int idx, int x, int y) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (nx >= 0 && nx < n && ny >= 0 && ny < m 
                && !visit[nx][ny] && board[nx][ny] == word[idx]) {
                visit[nx][ny] = true;
                backtrack(board, word, visit, idx + 1, nx, ny);
                visit[nx][ny] = false;
            }
        }
    }
...
注意思路，这里的d[i]代表的是第i家及之前所能截获的最多钱

递推公式，如果当前不抢，则获取金钱为dp[i-1]，若抢，则为dp[i-2]+nums[i]
    ...
    int numSquares(int n) {
        ...
        for(int i = 2;i<=n;i++){
            ...
            for(int j =1 ;j<=sqrt(i);j++){
                //递推公式，遍历i直到sqrt(i)
                if(res[i-j*j]<min) min = res[i-j*j];
            }
            res[i] = 1+min;
        }
...
public:
    bool wordBreak(string s, vector<string>& wordDict) {
        ...
        //核心思想，dp[i]表示前i个字符能不能被dict中的字符所表示
        //将wordDict转为set
        unordered_set<string> word;
        for(int i =0;i<wordDict.size();i++){
            word.insert(wordDict[i]);
        }
        vector<bool> bp = vector<bool>(s.size()+1,false);
        bp[0] = true;
        for(int i = 1;i<=s.size();i++){
            for(int j = 0;j<i;j++){
                string n = s.substr(j,i-j);//从j开始到i的字符串,eg.a.substr(0,1) == a[0] ,从0开始，长度为1
                if(word.find(n) != word.end() && bp[j]){
                    bp[i] = true;
                    break;
                }
            }
...
public:
    int longestCommonSubsequence(string text1, string text2) {
        ...
        if(text1.length() ==1 && text2.length() == 1) return text1[0] == text2[0];
...
public:
    vector<int> partitionLabels(string s) {
        //找到每一种字符距离自己最远的位置,合并区间
        int last[26];
        for(int i =0;i<s.length();i++){
            last[s[i]-'a'] = i;
        }
        vector<int> res;
        int end = 0 , start = 0;//当前字节终止位置和下一字节的起始位置
        for(int i =0 ;i<s.length();i++){
...
public:
    ...
    int search(vector<int>& nums, int target) {
        ...
        while(l<r){
            ...
            //左侧有序，且target在左侧
            if(nums[mid]>nums[0]){
                int res = bisearch(nums,target,l,mid);
                if(nums[res] == target) return res;
                l = mid+1;
            }
            else{
                int res = bisearch(nums,target,mid+1,r);
                if(nums[res] == target) return res;
                r = mid;
            }
        }
...
>   单调栈适用于寻找当前元素前/后第一个大于/小于该元素的数值/下标
>
>   在本题中，需要额外注意的是栈中存储的元素是下标而不是具体数值
